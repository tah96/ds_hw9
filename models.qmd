...
Below we see our table of coefficients for our best lasso model. This includes our estimates and our tuning parameter (penalty)

```{r}
lasso_summary <- lasso_wfl_3 |>
  finalize_workflow(lowest_rmse_3) |>
  fit(bike_train)
tidy(lasso_summary)
```
...
Below we can see our final variable plot for our bagged tree. This tells us the importance of our variables. We see that ____, ____ and ____ are our most important variables
...
We can see for both MAE and RMSE, our Random Forest model is the winner (first two rows) and boast the lowest values for each!
